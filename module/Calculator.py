'''
Python script to evaluate string as formula
From nerdparadise.com
'''
...
class Parser:
    ...
    def parseMultiplication(self):
        ...
        while True:
            ...
            char = self.peek()
            if char == '^':
                power_index = self.index
                self.index += 1
                exposant = self.parseParenthesis()
                values[-1] = values[-1]**exposant
            elif char == '*':
                self.index += 1
    ...
    def parseVariable(self):
        ...
        while self.hasNext():
            char = self.peek()
            if char.lower() in '_abcdefghijklmnopqrstuvwxyzρ0123456789':
                var += char
                self.index += 1
            else:
                break
