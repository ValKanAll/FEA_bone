...
class MainWindow(tk.Tk):
    def __init__(self, *args, ** kwargs):
        ...
        self.menu_file = ToolMenu(self.main_menu_bar, tearoff=0)
        self.menu_file.add_command(label="Open existing project", command=self.fun('open'))
        self.menu_file.add_command(label="Create new project", command=self.fun('create'))
        self.menu_file.add_command(label="Save Project", command=self.fun('save'))
        self.menu_file.add_separator()
        self.menu_file.add_command(label="Settings", command=self.setup)
        self.menu_file.add_separator()
        self.menu_file.add_command(label="Quit", command=self.quit)
        self.main_menu_bar.add_cascade(label="Project", menu=self.menu_file)

        self.menu_mesh = ToolMenu(self.main_menu_bar, tearoff=0)
        self.menu_mesh.add_command(label="New mesh", command=self.analyse, state=tk.DISABLED)
        self.main_menu_bar.add_cascade(label="Mesh", menu=self.menu_mesh)

        self.menu_mekamesh = ToolMenu(self.main_menu_bar, tearoff=0)
        self.menu_mekamesh.add_command(label="New mechanical mesh", command=self.analyse, state=tk.DISABLED)
        self.main_menu_bar.add_cascade(label="Mechanical mesh", menu=self.menu_mekamesh)

        self.menu_simulation = ToolMenu(self.main_menu_bar, tearoff=0)
        self.menu_simulation.add_command(label="New simulation", command=self.analyse, state=tk.DISABLED)
        self.main_menu_bar.add_cascade(label="Simulation", menu=self.menu_simulation)

        self.menu_about = ToolMenu(self.main_menu_bar, tearoff=0)
        self.menu_about.add_command(label="About", command=self.fun('*'))
        self.main_menu_bar.add_cascade(label="Help", menu=self.menu_about)
        ...
        self.export_button = tk.Button(self.frame_exportation, text="Export model", width=10, command=self.modify,
                                       state=tk.DISABLED)
        ...
        self.value.set("")
        self.entree = tk.Entry(self.frame_original, textvariable=self.value, show="Enter path of original model", width=20)
